...
#apply to tweets_LEFT collection - EXCEEDS MEMORY!! 
...
# now, from tweets_filtered collection, delete users whose user_id_str is in users_to_delete_LEFT.txt  
...
users_to_delete2[0] #'4667'
'4667' in users_to_delete2 #True 
...
1121578444943429632 in elites # currently item needs to be an integer
len(elites) #420
elites = [str(i) for i in elites] #converting each item to string
elites
type(elites[0]) #str
'1121578444943429632' in elites #True 
...
#### find number of tweets for very final sample: ####
sample = pd.read_csv('RESULTS_df_multiverse_6.csv', index_col=0)
sample.head()
sample_LEFT = sample[sample.side=='LEFT']
sample_LEFT #17788 rows × 20 columns

users_to_keep_LEFT = [str(i) for i in list(sample_LEFT.user_id_str)]

sample_RIGHT = sample[sample.side=='RIGHT']
sample_RIGHT.dropna() #16170 rows × 20 columns

users_to_keep_RIGHT = [str(i) for i in list(sample_RIGHT.user_id_str)]
...
db = client['TWITTER_LEFT']  ## NB REPEAT for RIGHT (below)
db.tweets_filtered.aggregate([
    {"$match" : {"user_id_str" : {"$in": users_to_keep_LEFT}}},
    {"$out" : "tweets_filtered_final" }
    ], allowDiskUse = True)

db.tweets_filtered_final.count() #3,090,937 tweets
len(db.tweets_filtered_final.distinct("user_id_str")) # ... unique users



db = client['TWITTER_RIGHT']  
db.tweets_filtered.aggregate([
    {"$match" : {"user_id_str" : {"$in": users_to_keep_RIGHT}}},
    {"$out" : "tweets_filtered_final" }
    ], allowDiskUse = True)

db.tweets_filtered_final.count() #2,280,722 tweets
len(db.tweets_filtered_final.distinct("user_id_str")) # ... unique users
