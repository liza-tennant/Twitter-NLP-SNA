...
################# 2 - apply ark TweetNLP (in command line, Java) #################
## script also saved in 'myCommandScript.command' and 'myCommandScript_CoNLL_format.command'

#COMMAND LINE input:

### cd /ark-tweet-nlp-0.3.2
### java -Xmx500m -jar ark-tweet-nlp-0.3.2.jar examples/example_tweets.txt
### java -Xmx500m -jar ark-tweet-nlp-0.3.2.jar inputs/LEFT/tweets_984522900_four.txt >> outputs/LEFT/tweets_984522900_four_tagged.txt
...
df_results.iloc[17786:17800,] #checkedt that they joined together correctly, ignoring the header 
...
import pyconll
pyconll.unit.conll.Conll(data)

filepath = 'ark-tweet-nlp-0.3.2/examples/covid_tagged_conll.conll'

tweets = pyconll.load_from_file(filepath)

Noun_lemmas = set()
Propernoun_lemmas = set()
for sentence in tweets:
    for token in sentence:
        if token.upos in ['^', 'Z', 'M']:
            Propernoun_lemmas.add(token.lemma)
        elif token.upos in ['N', 'S', 'L']:
            Noun_lemmas.add(toekn.lemma)
            



#read in tweets from single user - e.g. tweets_577.txt (LEFT)
with open('ark-tweet-nlp-0.3.2/outputs_conll/LEFT/tweets_577.txt', 'r') as f:
    data = f.read()
len(data) #33768 symbols


filename = 'ark-tweet-nlp-0.3.2/outputs_conll/LEFT/tweets_577.txt'
#5 tweets in this file
...
#can use parse_incr() if loading >1Mb of sentences at once 
sentences = parse(data, fields=["token", "tag", "confidence"])
sentences[0][0]
sentences
...
## the following will be REPEATED FOR EVERY USER IN outputs_conll folder: 
user_id = 577 ## later extract this from each object I loop through 
df = pd.DataFrame(columns=['token', 'tag', 'confidence'])
errors = []
tweets_count = 0
with open('ark-tweet-nlp-0.3.2/outputs_conll/LEFT/tweets_577.txt', 'r') as f:
    try:
        for tweet in f.read().split('\n\n'): #for every tweet from this user
            ...
            df_line = pd.DataFrame([x.split('\t') for x in lines], columns=['token', 'tag', 'confidence']) #create df for every tag (triple)
            df = df.append(df_line, ignore_index=True) #treat one user's tweets all together, as one long tweet
    except ValueError as e: 
        errors.append({tweet: e}) #need this to ignore the blank line at the end of the tweet file
    except AssertionError as e2: 
        errors.append({tweet: e2})
df #2321 rows × 3 columns
errors #1 item
